    ...
    export class GraphHandler implements IMouseListener {
        ...
        getGuideStates() : CellState[] {
            ...
            var filter = Utils.bind(this, cell => {
                return this.graph.view.getState(cell) != null &&
                    model.isVertex(cell) && Cells.getGeometry(cell) != null && !Cells.getGeometry(cell).relative;
            });
        ...
        private shouldRemoveCellsFromParent(parent: Cell, cells, evt: MouseEvent): boolean {
            if (this.graph.getModel().isVertex(parent)) {
                ...
                var pt = Utils.convertPoint(this.graph.container,
                    Events.getClientX(evt), Events.getClientY(evt));
                var alpha = Utils.toRadians(Utils.getFloat(pState.style, Constants.styleRotation, 0));
