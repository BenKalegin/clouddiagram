module Five {
    ...
    export class Graph {
        ...
        nativeDblClickEnabled: boolean = !Client.isQuirks && (document.documentMode == null || document.documentMode < 10);
        ...
        // EdgeStyle to be used for loops. This is a fallback for loops if the Constants.STYLE_LOOP is undefined. Default is EdgeStyle.Loop.
        defaultLoopStyle = "loop";
        ...
        * Note that <autoExtend> is disabled if this is true and that this should only be used with a scroll buffer or when scollbars are visible and scrollable in all directions. Default is false.*/
        ...
        lastMouseX: number;
        lastMouseY: number;
        shiftPreview1: HTMLDivElement;
        shiftPreview2: HTMLDivElement;
        ...
        constructor(container: HTMLElement, model?: GraphModel, renderHint?: RenderingHint, stylesheet?: Stylesheet) {
        ...
        init(container: HTMLElement) {
            ...
            if (Client.isIe) {
                ...
                Events.addListener(container, "selectstart",
                    Utils.bind(this, (evt) => {
                        return this.isEditing() || (!this.isMouseDown && !Events.isShiftDown(evt));
                    })
                );
            }
        ...
        isLabelClipped(cell: Cell) {
            ...
            return (style != null) ? style[Constants.styleOverflow] == "hidden" : false;
        }
        ...
        isWrapping(cell: Cell): boolean {
            ...
            return (style != null) ? style[Constants.styleWhiteSpace] === "wrap" : false;
        }
        ...
        */
        getCellStyle(cell: Cell): { [key: string]: string } {
        ...
        */
        ...
        getConnectionConstraint(edge: CellState, terminal: CellState, source: boolean): ConnectionConstraint {
            var point = null;
            var x = edge.style[(source) ? Constants.styleExitX : Constants.styleEntryX];

            if (x != null) {
                var y = edge.style[(source) ? Constants.styleExitY : Constants.styleEntryY];
            ...
            if (point != null) {
                perimeter = Utils.getBoolean(edge.style, (source) ? Constants.styleExitPerimeter : Constants.styleEntryPerimeter, true);
            }
        ...
        getConnectionPoint(vertex: CellState, constraint: ConnectionConstraint): Point {
            ...
            if (vertex != null) {
                ...
                var direction: Direction = Direction[Utils.getValue(vertex.style, Constants.styleDirection, null)];
                var r1 = 0;
                ...
                if (constraint.point != null) {
                    ...
                    if (this.getModel().isVertex(vertex.cell)) {
                        var flipH = vertex.style[Constants.styleFlipH] == "1";
                        var flipV = vertex.style[Constants.styleFlipV] == "1";

                        // Legacy support for stencilFlipH/V
                        if (vertex.shape != null && vertex.shape.stencil != null) {
                            flipH = Utils.getBoolean(vertex.style, "stencilFlipH") || flipH;
                            flipV = Utils.getBoolean(vertex.style, "stencilFlipV") || flipV;
                        }
                ...
                var r2 = Number(vertex.style[Constants.styleRotation]) || 0;
                var cos: number;
                var sin: number;
        ...
        postProcessCellStyle(style: { [index: string]: string; }): { [index: string]: string; } {
            ...
            if (style != null) {
                var key: string = style[Constants.styleImage];
                var image: string = this.getImageFromBundles(key);

                if (image != null) {
                    style[Constants.styleImage] = image;
                } else {
                    image = key;
                }
        ...
        isOrthogonal(edge: CellState): boolean {
            var orthogonal: string = edge.style[Constants.styleOrthogonal];
        ...
        private getStateForTouchEvent(evt: MouseEvent): CellState {
            ...
            var pt = Utils.convertPoint(this.container, x, y);
...
/**
        ...
        /**
         * Shifts the graph display by the given amount. This is used to preview panning operations, use <mxGraphView.setTranslate> to set a persistent
         * translation of the view. Fires <mxEvent.PAN>.
         * dx - Amount to shift the graph along the x-axis.
         * dy - Amount to shift the graph along the y-axis.
         */
        panGraph(dx: number, dy: number) {
            if (this.useScrollbarsForPanning && Utils.hasScrollbars(this.container)) {
                ...
                if (this.dialect == Dialect.Svg) {
                    ...
                    if (dx == 0 && dy == 0) {
                        ...
                        if (this.shiftPreview1 != null) {
                            child = this.shiftPreview1.firstChild;
                            while (child != null) {
                        ...
                        if (this.shiftPreview1 == null) {
                            ...
                            while (child != null) {
                                next = child.nextSibling; // SVG element is moved via transform attribute
                ...
                if (x >= c.scrollLeft && y >= c.scrollTop && x <= c.scrollLeft + c.clientWidth &&
                    ...
                    if (dx < border) {
                        ...
                        if (extend && old == c.scrollLeft) {
                            ...
                            if (this.dialect == Dialect.Svg) {
                                ...
                                width = this.container.scrollWidth + border - dx; // Updates the clipping region. This is an expensive
                    ...
                    if (dy < border) {
                        ...
                        if (old == c.scrollTop && extend) {
                            ...
                            if (this.dialect == Dialect.Svg) {
                                root = (<SVGElement>this.view.getDrawPane()).ownerSVGElement;
                                height = this.container.scrollHeight + border - dy; // Updates the clipping region. This is an expensive
                                // operation that should not be executed too often.
                                root.style.height = height + "px";
                            } else {
                                height = Math.max(c.clientHeight, c.scrollHeight) + border - dy;
                                canvas = this.view.getCanvas();
        ...
        fireMouseEvent(evtName: string, me: MouseEventContext, sender?: Object) {
            ...
            if (!this.isEventIgnored(evtName, me, sender)) {
                ...
                (Client.isIe && Client.isSvg) || me.getEvent().target != this.container)) {
                    ...
                    if (this.mouseListeners != null) {
                        ...
                        if (!me.getEvent().preventDefault) {
                            (<MSEventObj><any>me.getEvent()).returnValue = true;
                        }
        ...
        */
        ...
        getIndicatorShape(state: CellState): string {
        ...
        /**
         * Returns the image URL for the given cell state. This implementation returns the value stored under <Constants.STYLE_IMAGE> in the cell style.
         * state - <mxCellState> whose image URL should be returned.
         */
        getImage(state: CellState): string {
            return (state != null && state.style != null) ? state.style[Constants.styleImage] : null;
        }

        /**
        * Returns the indicator color for the given cell state. This implementation returns the value stored under <Constants.STYLE_INDICATOR_COLOR> in the cell style.
        * Parameters:
        * state - <mxCellState> whose indicator color should bereturned.
        */
        getIndicatorColor(state: CellState) {
            return (state != null && state.style != null) ? state.style[Constants.styleIndicatorColor] : null;
        }

        /**
        * Returns the indicator gradient color for the given cell state. This implementation returns the value stored under <Constants.STYLE_INDICATOR_GRADIENTCOLOR> in the cell style. 
        * Parameters:
        * 
        * state - <mxCellState> whose indicator gradient color should be returned.
        */
        getIndicatorGradientColor(state: CellState): string {
            return (state != null && state.style != null) ? state.style[Constants.styleIndicatorGradientcolor] : null;
        }

        /**
        * Returns the indicator image for the given cell state. This implementation returns the value stored under
        * <Constants.STYLE_INDICATOR_IMAGE> in the cell style.        * 
        * Parameters:
        * state - <mxCellState> whose indicator image should be returned.
        */
        getIndicatorImage(state: CellState) {
            return (state != null && state.style != null) ? state.style[Constants.styleIndicatorImage] : null;
        }
            ...
            if (cell != null) {
                if (this.model.getParent(cell) != this.model.getRoot()) {
                    ...
                    if (style != null && !this.model.isEdge(cell)) {
                        return style[Constants.styleShape] == Constants.shapeSwimlane;
                    }
                }
            }
            return false;
...
/**
        ...
        getLabel(cell: Cell): string {
            ...
            if (this.labelsVisible && cell != null) {
                ...
                if (!Utils.getBoolean(style, Constants.styleNolabel, false)) {
        ...
        convertValueToString(cell) {
            var value = <Element>this.model.getValue(cell);
        ...
        getBorderSizes(): Rectangle {
            ...
            var style = Utils.getCurrentStyle(this.container);
        ...
        getVerticalAlign(state: CellState): string {
            return (state != null && state.style != null) ?
            (state.style[Constants.styleVerticalAlign] || Constants.alignMiddle) : null;
        }
        ...
        */
        ...
        getOutlineConstraint(point: Point, terminalState: CellState, me: MouseEventContext): ConnectionConstraint {
            if (terminalState.shape != null) {
                ...
                var direction = Direction[terminalState.style[Constants.styleDirection]];
                ...
                if (this.getModel().isVertex(terminalState.cell)) {
                    var flipH = terminalState.style[Constants.styleFlipH] == "1";
                    var flipV = terminalState.style[Constants.styleFlipV] == "1";

                    // Legacy support for stencilFlipH/V
                    if (terminalState.shape != null && terminalState.shape.stencil != null) {
                        flipH = Utils.getBoolean(terminalState.style, "stencilFlipH", flipH);
                        flipV = Utils.getBoolean(terminalState.style, "stencilFlipV", flipV);
                    }
        ...
        setConnectionConstraint(edge: Cell, terminal: Cell, source: boolean, constraint: ConnectionConstraint) {
            ...
            if (constraint != null) {
                ...
                try {
                    if (constraint.point == null) {
                        this.setCellStyles((source) ? Constants.styleExitX : Constants.styleEntryX, null, [edge]);
                        this.setCellStyles((source) ? Constants.styleExitY : Constants.styleEntryY, null, [edge]);
                        this.setCellStyles((source) ? Constants.styleExitPerimeter : Constants.styleEntryPerimeter, null, [edge]);
                    } else {
                        this.setCellStyles((source) ? Constants.styleExitX : Constants.styleEntryX, "" + constraint.point.x, [edge]);
                        this.setCellStyles((source) ? Constants.styleExitY : Constants.styleEntryY, "" + constraint.point.y, [edge]);

                        // Only writes 0 since 1 is default
                        if (!constraint.perimeter) {
                            this.setCellStyles((source) ? Constants.styleExitPerimeter : Constants.styleEntryPerimeter, "0", [edge]);
                        } else {
                            this.setCellStyles((source) ? Constants.styleExitPerimeter : Constants.styleEntryPerimeter, null, [edge]);
                        }
                    }
        ...
        setCellStyles(key: string, value: string, cells?: Cell[]) {
            ...
            if (state != null) {
                if (this.model.isEdge(state.cell)) {
                    ...
                    if (this.isLoop(state) ||
                        ...
                        result = new ElbowEdgeHandler(state);
                    } else if (style == EdgeStyle.segmentConnector ||
                        style == EdgeStyle.orthConnector) {
                        result = new EdgeSegmentHandler(state);
                    } else {
                        result = new EdgeHandler(state);
                    }
                } else {
                    result = new VertexHandler(state);
                }
            }
        ...
        getStartSize(swimlane: Cell): Rectangle {
            ...
            if (style != null) {
                var size = Utils.getInt(style, Constants.styleStartsize, Constants.defaultStartsize);

                if (Utils.getBoolean(style, Constants.styleHorizontal, true)) {
                    result.height = size;
                } else {
                    result.width = size;
                }
        ...
        getPointForEvent(evt: MouseEvent, addOffset: boolean = true): Point {
            ...
            var p = Utils.convertPoint(this.container, Events.getClientX(evt), Events.getClientY(evt));
        ...
        isCellMovable(cell: Cell): boolean {
            ...
            return this.isCellsMovable() && !this.isCellLocked(cell) && Utils.getBoolean(style, Constants.styleMovable, true);
        }
        ...
        getDropTarget(cells: Cell[], evt: MouseEvent, cell: Cell, clone?: boolean): Cell {
            ...
            var pt = Utils.convertPoint(this.container, Events.getClientX(evt), Events.getClientY(evt));
            pt.x -= this.panDx;
            pt.y -= this.panDy;
        ...
        insertEdge(parent: Cell, id: number, value: any, source: Cell, target: Cell, style?: string): Cell {
        ...
        scrollCellToVisible(cell: Cell, center?: boolean) {
            ...
            if (state != null) {
                var bounds = new Rectangle(x + state.x, y + state.y, state.width,
                    state.height);
        ...
        private zoom(factor: number, center?: boolean) {
            ...
            if (this.keepSelectionVisibleOnZoom && state != null) {
                ...
                var hasScrollbars = Utils.hasScrollbars(this.container);
                var dx: number;
                var dy: number;
                if (center && !hasScrollbars) {
                    dx = this.container.offsetWidth;
                    dy = this.container.offsetHeight;
                    var f: number;
                    if (factor > 1) {
                        f = (factor - 1) / (scale * 2);
                        dx *= -f;
        ...
        isCellFoldable(cell: Cell, collapse: boolean): boolean {
            ...
            return this.model.getChildCount(cell) > 0 && style[Constants.styleFoldable] != "0";
        }
        ...
        isAutoSizeCell(cell: Cell): boolean {
            ...
            return this.isAutoSizeCells() || style[Constants.styleAutosize] == "1";
        }
        ...
        isCellResizable(cell: Cell): boolean {
            ...
            return this.isCellsResizable() && !this.isCellLocked(cell) && Utils.getBoolean(style, Constants.styleResizable, true);
        }
        ...
        createEdge(parent: Cell, id: number, value: Node, source: Cell, target: Cell, style: string): Cell {
                ...
                try {
                    ...
                    if (size != null && geo != null) {
                        ...
                        if (this.isSwimlane(cell)) {
                            ...
                            if (Utils.getBoolean(style, Constants.styleHorizontal, true)) {
                                cellStyle = Utils.setStyle(cellStyle, Constants.styleStartsize, size.height + 8);
                                ...
                                cellStyle = Utils.setStyle(cellStyle, Constants.styleStartsize, size.width + 8);
        ...
        private isAutoSizeCells(): boolean {
        ...
        private isCellsResizable(): boolean {
        ...
        updateAlternateBounds(cell: Cell, geo: Geometry, willCollapse: boolean) {
            ...
            if (cell != null && geo != null) {
                ...
                if (geo.alternateBounds == null) {
                    ...
                    if (this.collapseToPreferredSize) {
                        ...
                        if (tmp != null) {
                            ...
                            var startSize = Utils.getInt(style, Constants.styleStartsize, 0);
                ...
                if (geo.alternateBounds != null) {
                    ...
                    var alpha = Utils.toRadians(Number(style[Constants.styleRotation] || "0"));
            ...
            if (geo != null) {
                ...
                if (geo.relative && !this.model.isEdge(cell)) {
                    ...
                    if (this.model.isVertex(parent)) {
                        ...
                        angle = Utils.getFloat(style, Constants.styleRotation, 0);
                    }
        ...
        private getPreferredSizeForCell(cell: Cell): Rectangle {
            ...
            var getVertexPrefferedSize = () : Rectangle => {
                ...
                var fontSize = Utils.getFloat(style, Constants.styleFontsize, Constants.defaultFontSize);
                var dx = 0;
                var dy = 0;
                if (this.getImage(state) != null || style[Constants.styleImage] != null) {
                    if (style[Constants.styleShape] == Constants.shapeLabel) {
                        if (style[Constants.styleVerticalAlign] == Constants.alignMiddle) {
                            dx += parseFloat(style[Constants.styleImageWidth]) || LabelShape.imageSize;
                        }

                        if (style[Constants.styleAlign] != Constants.alignCenter) {
                            dy += parseFloat(style[Constants.styleImageHeight]) || LabelShape.imageSize;
                        }
                    }
                }
                dx += 2 * Utils.getInt(style, Constants.styleSpacing, 0);
                dx += Utils.getInt(style, Constants.styleSpacingLeft, 0);
                dx += Utils.getInt(style, Constants.styleSpacingRight, 0);
                dy += 2 * Utils.getInt(style, Constants.styleSpacing, 0);
                dy += Utils.getInt(style, Constants.styleSpacingTop, 0);
                dy += Utils.getInt(style, Constants.styleSpacingBottom, 0);
                ...
                if (value != null && value.length > 0) {
                    ...
                    var maxLabelWidth = cell.geometry.maxWidth - dx;
                    var size = Utils.getSizeForString(value, fontSize, style[Constants.styleFontfamily], maxLabelWidth);
            ...
            var result = this.model.isEdge(cell) ? getEdgePrefferedSize() : getVertexPrefferedSize();
                ...
                if (parent == this.getDefaultParent() || parent == this.getCurrentRoot()) {
                    ...
                    if (g != null) {
                        ...
                        if (this.isSwimlane(parent)) {
                            ...
                            var state = this.view.getState(parent);
                            var style = (state != null) ? state.style : this.getCellStyle(parent);
                            var dir = Direction[style[Constants.styleDirection]] || Direction.East;
                            var flipH = Utils.getBoolean(style, Constants.styleFlipH, false);
                            var flipV = Utils.getBoolean(style, Constants.styleFlipV, false);
                            ...
                            if ((dir == Direction.East && !flipV) || (dir == Direction.North && !flipH) ||
                ...
                if (parent != null) {
                    ...
                    for (var i = 0; i < childCount; i++) {
                        ...
                        if (this.isCellVisible(cell) && state != null) {
                            var box: Rectangle;
                            box = state;
                            var deg = Utils.getFloat(state.style, Constants.styleRotation, 0);
        ...
        insertVertex(parent: Cell, id: number, value: string, x: number, y: number, width: number, height: number, style?: string, relative: boolean = false, sizeRestriction?: ICellSizeRestrictions) {
             ...
             * Default is false.
             */
            var vertex = this.createVertex(parent, id, value, x, y, width, height, style, relative, sizeRestriction);
            return this.addCell(vertex, parent);
        }

        /** Hook method that creates the new vertex for <insertVertex>. */
        createVertex(parent: Cell, id: number, value: string, x: number, y: number, width: number, height: number, style: string, relative: boolean, sizeRestriction?: ICellSizeRestrictions) {
            // Creates the geometry for the vertex
            var geometry = new Geometry(x, y, width, height, sizeRestriction);
...
// Adds or removes a child to the view by online invaliding the minimal required portions of the cache, namely, the old and new parent and the child.
                ...
                var newParent = this.model.getParent(child);
...
// Removes the state from the cache by default
        ...
        private selectCell(isNext: boolean = false, isParent: boolean = false, isChild: boolean = false) {
            ...
            if (cell == null && childCount > 0) {
                child = this.model.getChildAt(parent, 0);
                ...
                if (tmp > 0) {
                    child = this.model.getChildAt(cell, 0);
                    this.setSelectionCell(child);
                }
                ...
                if (isNext) {
                    i++;
                    child = this.model.getChildAt(parent, i % childCount);
                    this.setSelectionCell(child);
                } else {
                    i--;
                    var index = (i < 0) ? childCount - 1 : i;
                    child = this.model.getChildAt(parent, index);
                    this.setSelectionCell(child);
                }
            }
        }
        ...
        toggleCellStyleFlags(key: string, flag: number, cells?: Cell[]) {
         ...
         * cells - Optional array of <mxCells> to change the style for. Default is the selection cells. */
        setCellStyleFlags(key: string, flag: number, value: boolean, cells: Cell[]) {
            cells = cells || this.getSelectionCells();
            ...
            if (cells != null && cells.length > 0) {
                ...
                Utils.setCellStyleFlags(this.model, cells, key, flag, value);
            }
        }
        ...
        alignCells(align: string, cells?: Cell[], param?: number) {
            if (cells == null) {
                cells = this.getSelectionCells();
            }
            ...
            if (cells != null && cells.length > 1) {
                    ...
                    for (i = 0; i < cells.length; i++) {
                        ...
                        if (state != null && !this.model.isEdge(cells[i])) {
                            if (param == null) {
                                ...
                                } else if (align == Constants.alignTop) {
                                    param = state.y;
                                } else if (align == Constants.alignMiddle) {
                                    param = state.y + state.height / 2;
                                    break;
                                } else if (align == Constants.alignBottom) {
                ...
                if (param != null) {
                    ...
                    this.onAlignCells.fire(new AlignCellsEvent(cells, align));
                    } finally {
                        this.model.endUpdate();
                    }
                }
