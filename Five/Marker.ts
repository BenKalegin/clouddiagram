module Five {
    interface IMarkerPaint {
        (canvas: AbstractCanvas2D, shape: Shape, type: string, pe: Point, unitX: number, unitY: number, size: number, source: boolean, sw : number, filled : boolean): () => void;        
    }
    ...
    export class Marker {
        ...
        static createMarker(canvas: AbstractCanvas2D, shape: Shape, type: string, pe: Point, unitX: number, unitY: number, size: number, source, sw: number, filled: boolean) {
        ...
        private static arrow(canvas: AbstractCanvas2D, shape: Shape, type: string, pe: Point, unitX: number, unitY: number, size: number, source: boolean, sw: number, filled: boolean): () => void {
            ...
            var f = (type != Constants.arrowClassic) ? 1 : 3 / 4;
            ...
            return () => {
                ...
                if (type == Constants.arrowClassic) {
        ...
        private static diamond(canvas: AbstractCanvas2D, shape: Shape, type: string, pe: Point, unitX: number, unitY: number, size: number, source: boolean, sw: number, filled: boolean): () => void {
            // The angle of the forward facing arrow sides against the x axis is
            // 45 degrees, 1/sin(45) = 1.4142 / 2 = 0.7071 ( / 2 allows for
            // only half the strokewidth is processed ). Or 0.9862 for thin diamond.
            // Note these values and the tk variable below are dependent, update
            // both together (saves trig hard coding it).
            var swFactor = (type == Constants.arrowDiamond) ? 0.7071 : 0.9862;
            ...
            var tk = ((type == Constants.arrowDiamond) ? 2 : 3.4);
