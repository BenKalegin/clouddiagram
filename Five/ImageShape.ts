...
module Five {
    export class ImageShape extends RectangleShape {
        /// <summary> Extends Shape to implement an image shape. This shape is registered under Constants.SHAPE_IMAGE in xCellRenderer.</summary>
         ...
         * respective values from <Constants.STYLE_IMAGE_BACKGROUND> and
        ...
        apply(state: CellState) {
            ...
            if (this.style != null) {
                this.preserveImageAspect = Utils.getInt(this.style, Constants.styleImageAspect, 1) === 1;

                // Legacy support for imageFlipH/V
                this.flipH = this.flipH || Utils.getBoolean(this.style, "imageFlipH");
                this.flipV = this.flipV || Utils.getBoolean(this.style, "imageFlipV");
            }
        }
        ...
        paintVertexShape(c: AbstractCanvas2D, x: number, y: number, w: number, h: number) {
            if (this.image != null) {
                var fill = Utils.getValue(this.style, Constants.styleImageBackground, null);
                var stroke = Utils.getValue(this.style, Constants.styleImageBorder, null);
        ...
        redrawHtmlShape() {
            ...
            if (this.image != null) {
                var fill = Utils.getValue(this.style, Constants.styleImageBackground, "");
                var stroke = Utils.getValue(this.style, Constants.styleImageBorder, "");
