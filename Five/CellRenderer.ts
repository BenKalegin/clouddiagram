...
module Five {
    ...
    export class CellRenderer {
        ...
        static defaultShapes: { [key: string]: () => Shape} = {};


        static registerShape(key: string, shape: () => Shape) {
            CellRenderer.defaultShapes[key] = shape;
        }

        static registerDefaultShapes(){
            // Adds default shapes into the default shapes array
            CellRenderer.registerShape(Constants.shapeRectangle,  RectangleShape.factory);
            CellRenderer.registerShape(Constants.shapeEllipse, EllipseShape.factory);
            CellRenderer.registerShape(Constants.shapeRhombus, RhombusShape.factory);
            CellRenderer.registerShape(Constants.shapeCylinder, CylinderShape.factory);
            CellRenderer.registerShape(Constants.shapeConnector, ConnectorShape.factory);
            CellRenderer.registerShape(Constants.shapeActor, ActorShape.factory);
            CellRenderer.registerShape(Constants.shapeTriangle, TriangleShape.factory);
            CellRenderer.registerShape(Constants.shapeHexagon, HexagonShape.factory);
            CellRenderer.registerShape(Constants.shapeCloud, CloudShape.factory);
            CellRenderer.registerShape(Constants.shapeLine, LineShape.factory);
            CellRenderer.registerShape(Constants.shapeArrow, ArrowShape.factory);
            CellRenderer.registerShape(Constants.shapeDoubleEllipse, DoubleEllipseShape.factory);
            CellRenderer.registerShape(Constants.shapeSwimlane, SwimlaneShape.factory);
            CellRenderer.registerShape(Constants.shapeImage, ImageShape.factory);
            CellRenderer.registerShape(Constants.shapeLabel, LabelShape.factory);
        }
        /**
        ...
        createShape(state: CellState) {
            if (state.style != null) {
                ...
                var stencil = StencilRegistry.getStencil(state.style[Constants.styleShape]);
        ...
        getShape(name: string): () => Shape {
            return (name != null) ? CellRenderer.defaultShapes[name] : null;
        }

        /** Returns the constructor to be used for creating the shape. */
        getShapeConstructor(state: CellState) : () => Shape {
            var ctor = this.getShape(state.style[Constants.styleShape]);
        ...
        configureShape(state: CellState) {
            ...
            state.shape.indicatorStrokeColor = state.style[Constants.styleIndicatorStrokecolor];
            state.shape.indicatorGradientColor = state.view.graph.getIndicatorGradientColor(state);
            state.shape.indicatorDirection = Direction[state.style[Constants.styleIndicatorDirection]];
        ...
        postConfigureShape(state: CellState) {
            if (state.shape != null) {
                this.resolveColor(state, "indicatorColor", Constants.styleFillcolor);
                this.resolveColor(state, "indicatorGradientColor", Constants.styleGradientcolor);
                this.resolveColor(state, "fill", Constants.styleFillcolor);
                this.resolveColor(state, "stroke", Constants.styleStrokecolor);
                this.resolveColor(state, "gradient", Constants.styleGradientcolor);
            }
        }
         ...
         */
        private resolveColor(state: CellState, field, key) {
        ...
        private createLabel(state: CellState, value) {
            var graph = state.view.graph;
            if (parseFloat(state.style[Constants.styleFontsize]) > 0 || state.style[Constants.styleFontsize] == null) {
                ...
                state.text = new TextShape(value, new Rectangle(0, 0),
                    (state.style[Constants.styleAlign] || Constants.alignCenter),
                    graph.getVerticalAlign(state),
                    state.style[Constants.styleFontcolor],
                    state.style[Constants.styleFontfamily],
                    Utils.getInt(state.style, Constants.styleFontsize),
                    Utils.getInt(state.style, Constants.styleFontstyle),
                    Utils.getInt(state.style, Constants.styleSpacing),
                    Utils.getInt(state.style, Constants.styleSpacingTop),
                    Utils.getInt(state.style, Constants.styleSpacingRight),
                    Utils.getInt(state.style, Constants.styleSpacingBottom),
                    Utils.getInt(state.style, Constants.styleSpacingLeft),
                    Utils.getBoolean(state.style, Constants.styleHorizontal, true),
                    state.style[Constants.styleLabelBackgroundcolor],
                    state.style[Constants.styleLabelBordercolor],
                    graph.isWrapping(state.cell) && graph.isHtmlLabel(state.cell),
                    graph.isLabelClipped(state.cell),
                    state.style[Constants.styleOverflow],
                    Utils.getInt(state.style, Constants.styleLabelPadding));
                state.text.opacity = Utils.getInt(state.style, Constants.styleTextOpacity, 100);
                    ...
                    Utils.bind(this, evt => {
                        if (this.isLabelEvent(state, evt)) {
                            ...
                            forceGetCell = graph.dialect != Dialect.Svg &&
                                Events.getSource(evt).nodeName == "IMG";
                        }
                    }),
                    Utils.bind(this, evt => {
                        if (this.isLabelEvent(state, evt)) {
        ...
        private installListeners(state: CellState) {
            ...
            var getState = evt => {
                ...
                if ((graph.dialect != Dialect.Svg && Events.getSource(evt).nodeName == "IMG") || Client.isTouch) {
                    var x = Events.getClientX(evt);
                    var y = Events.getClientY(evt);
        ...
        private getLabelBounds(state: CellState): Rectangle {
            ...
            var lw = Utils.getValue(state.style, Constants.styleLabelWidth, null);
        ...
        private rotateLabelBounds(state: CellState, bounds: Rectangle) {
            ...
            if (state.style[Constants.styleOverflow] != "fill" && state.style[Constants.styleOverflow] != "width")
			{
                ...
                var hpos = Utils.getValue(state.style, Constants.styleLabelPosition, Constants.alignCenter);
                var vpos = Utils.getValue(state.style, Constants.styleVerticalLabelPosition, Constants.alignMiddle);
                var lw = Utils.getInt(state.style, Constants.styleLabelWidth, null);

                bounds.width = Math.max(0, bounds.width - ((hpos == Constants.alignCenter && lw == null) ? (state.text.spacingLeft * s + state.text.spacingRight * s) : 0));
                bounds.height = Math.max(0, bounds.height - ((vpos == Constants.alignMiddle) ? (state.text.spacingTop * s + state.text.spacingBottom * s) : 0));
            }
        ...
        private redrawCellOverlays(state: CellState, forced: boolean) {
            ...
            if (state.overlays != null) {
                var rot = Utils.mod(Utils.getFloat(state.style, Constants.styleRotation, 0), 90);
        ...
        private redrawControl(state: CellState, forced: boolean) {
            ...
            if (state.control != null && image != null) {
                ...
                var r = (this.legacyControlPosition) ?
                    Utils.getFloat(state.style, Constants.styleRotation, 0) :
                ...
                if (!state.view.graph.getModel().isEdge(state.cell)) {
                    ...
                    if (state.shape != null) {
                        ...
                        if (this.legacyControlPosition) {
                            rot = Utils.getFloat(state.style, Constants.styleRotation, 0);
                        } else {
