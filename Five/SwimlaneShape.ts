...
module Five {
    export class SwimlaneShape extends Shape {
        ...
        getTitleSize(): number {
            return Math.max(0, Utils.getInt(this.style, Constants.styleStartsize, Constants.defaultStartsize));
        }
        ...
        getLabelBounds(rect: Rectangle) {
            ...
            var flipH = Utils.getBoolean(this.style, Constants.styleFlipH, false);
            var flipV = Utils.getBoolean(this.style, Constants.styleFlipV, false);
        ...
        isHorizontal(): boolean {
            return Utils.getBoolean(this.style, Constants.styleHorizontal, true);
        }

        paintVertexShape(c: AbstractCanvas2D, x: number, y: number, w: number, h: number) {
            var start = this.getTitleSize();
            var fill = Utils.getValue(this.style, Constants.styleSwimlaneFillcolor, Constants.none);
            var swimlaneLine = Utils.getBoolean(this.style, Constants.styleSwimlaneLine, true);
            var r = 0;
            ...
            var sep = Utils.getValue(this.style, Constants.styleSeparatorcolor, Constants.none);
