module Five {
    export class VertexHandler implements ICellHandler {
        ...
        private init() {
            ...
            this.selectionBorder.dialect = (this.graph.dialect != Dialect.Svg) ? Dialect.Vml : Dialect.Svg;
            this.selectionBorder.pointerEvents = false;
            this.selectionBorder.rotation = Number(this.state.style[Constants.styleRotation] || "0");
            ...
            if (this.graph.isEnabled() && this.rotationEnabled && this.graph.isCellRotatable(this.state.cell) &&
                ...
                this.state.width > 2 && this.state.height > 2) {
                this.rotationShape = this.createSizer(this.rotationCursor, EventHandle.Rotation, Constants.handleSize + 3, Constants.handleFillcolor);
        ...
        private createSelectionShape(bounds: Rectangle): Shape {
            var shape = new RectangleShape(bounds, null, this.getSelectionColor());
        ...
        private getSelectionColor(): string {
            return Constants.vertexSelectionColor;
        }

        private getSelectionStrokeWidth(): number {
            return Constants.vertexSelectionStrokewidth;
        }

        private isSelectionDashed() : boolean {
            return Constants.vertexSelectionDashed;
        }
        ...
        private createSizer(cursor: string, index: EventHandle, size?: number, fillColor?: string) {
            size = size || Constants.handleSize;
        ...
        private createSizerShape(bounds: Rectangle, index: EventHandle, fillColor: string): Shape {
            if (this.handleImage != null) {
                ...
                return new EllipseShape(bounds, fillColor || Constants.handleFillcolor, Constants.handleStrokecolor);
            }
            else {
                return new RectangleShape(bounds, fillColor || Constants.handleFillcolor, Constants.handleStrokecolor);
            }
        }
        ...
        private start(x: number, y: number, index: EventHandle) {
            ...
            if (!this.livePreview || this.isLivePreviewBorder()) {
                ...
                if (!(Client.isSvg && Number(this.state.style[Constants.styleRotation] || "0") != 0) &&
            ...
            if (!me.isConsumed() && this.index != null) {
                ...
                if (!this.inTolerance) {
                    ...
                    if (this.index == EventHandle.Label) {
                        ...
                        var alpha = Utils.toRadians(Number(this.state.style[Constants.styleRotation] || "0"));
        ...
        mouseUp(sender: Object, me: MouseEventContext) {
            if (this.index != null && this.state != null) {
                ...
                try {
                    var rotation = Number(this.state.style[Constants.styleRotation] || "0");
            ...
            if (angle != 0) {
                ...
                if (model.isVertex(cell) || model.isEdge(cell)) {
                    if (!model.isEdge(cell)) {
                        ...
                        if (style != null) {
                            var total = Number(style[Constants.styleRotation] || "0") + angle;
                            this.graph.setCellStyles(Constants.styleRotation, "" + total, [cell]);
                        }
                    }
        ...
        private redrawHandles() {
            ...
            if (this.sizers != null) {
                ...
                if (this.singleSizer) {
                    ...
                    if (this.sizers.length > 1) {
                        ...
                        alpha = Utils.toRadians(Number(this.state.style[Constants.styleRotation] || "0"));
                        cos = Math.cos(alpha);
                        sin = Math.sin(alpha);
            ...
            if (this.rotationShape != null) {
                alpha = Utils.toRadians((this.currentAlpha != null) ? this.currentAlpha : Number(this.state.style[Constants.styleRotation] || "0"));
                cos = Math.cos(alpha);
                sin = Math.sin(alpha);
        ...
        private drawPreview() {
            if (this.preview != null) {
                ...
                this.preview.rotation = Number(this.state.style[Constants.styleRotation] || "0");
                this.preview.redraw();
            }
