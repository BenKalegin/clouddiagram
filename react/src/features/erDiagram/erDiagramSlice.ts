...
class ErDiagramHandler extends StructureDiagramHandler {
    ...
    connectNodes(get: Get, set: Set, sourceId: Id, target: ElementRef, _diagramPos: Coordinate): void {
        autoConnectErEntities(get, set, sourceId, target);
    }
}

const autoConnectErEntitiesImpl = (get: Get, set: Set, sourceId: Id, target: ElementRef) => {
...
const autoConnectErEntities = withHistory(autoConnectErEntitiesImpl, "Connect ER Entities");

export const erDiagramEditor = new ErDiagramHandler();
