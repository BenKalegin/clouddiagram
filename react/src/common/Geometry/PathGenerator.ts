...
export const GetRouteWithCurvePoints = (route: Coordinate[], source: PortState, sourcePlacement: PortPlacement,
...
export const Bezier = (route: Coordinate[], source: PortState, sourceBounds: Bounds, sourcePlacement: PortPlacement,
    ...
    if (route.length > 2) {
        return CurveThroughPoints(route).path
    }
...
export const Direct = (route: Coordinate[], source: PortState, sourceBounds: Bounds, sourcePlacement: PortPlacement,
...
export const LateralHorizontal = (route: Coordinate[], source: PortState, sourceBounds: Bounds, sourcePlacement: PortPlacement,
                                   target: PortState, targetBounds: Bounds, targetPlacement: PortPlacement) => {
    route = concatRouteAndSourceAndTarget(route, sourceBounds, targetBounds);

    if (Math.abs(route[0].x - route[1].x) < _margin * 2 &&
        Math.abs(route[0].y - route[1].y) < _margin * 2) {
        return Bezier([], source, sourceBounds, sourcePlacement, target, targetBounds, targetPlacement);
    }
...
export const LateralVertical = (route: Coordinate[], source: PortState, sourceBounds: Bounds, sourcePlacement: PortPlacement,
                                 target: PortState, targetBounds: Bounds, targetPlacement: PortPlacement) => {
    route = concatRouteAndSourceAndTarget(route, sourceBounds, targetBounds);
...
export const generatePath = (route: Coordinate[], source: PortState, sourceBounds: Bounds, sourcePlacement: PortPlacement,
                           target: PortState, targetBounds: Bounds, targetPlacement: PortPlacement, 
                           linkStyle: RouteStyle, tipStyle1: TipStyle, tipStyle2: TipStyle): string[] => {
    switch (linkStyle) {
        case RouteStyle.Direct:
            return Direct(route, source, sourceBounds, sourcePlacement, target, targetBounds, targetPlacement, tipStyle1, tipStyle2);
        case RouteStyle.Bezier:
            return Bezier(route, source, sourceBounds, sourcePlacement, target, targetBounds, targetPlacement);
