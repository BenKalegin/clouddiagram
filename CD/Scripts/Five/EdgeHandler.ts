module Five {
    ...
    */
    export class EdgeHandler implements ICellHandler {
        constructor(state: CellState, protected selHandleConfig: ISelectionHandleConfig) {
            if (state != null) {
                this.state = state;
                this.init();
        ...
        private init() {
            ...
            this.shape.init(ElementInitializer(this.graph.getView().getOverlayPane()));
        ...
        private createMarker() : CellMarker {
            ...
            marker.getCell = (me: MouseEventContext): Cell => {
                var cell = <Cell>CellMarker.prototype.getCell.apply(this, me);
	            ...
	            if ((this.graph.isSwimlane(cell) && this.graph.hitsSwimlaneContent(cell, point.x, point.y)) ||
                    ...
                    (cell == self.state.cell || (cell != null && !self.graph.connectableEdges && Cells.isEdge(cell))) ||
                    Cells.isAncestor(self.state.cell, cell)) {
                    cell = null;
                }
            ...
            marker.isValidState = (state: CellState) => {
                var other = this.graph.view.getTerminalPort(state, this.graph.view.getState(Cells.getTerminal(this.state.cell, !this.isSource)), !this.isSource);
        ...
        createHandleShape(index?: number) : RectangleShape {
            if (this.handleImage != null) {
                ...
                return new RectangleShape(new Rectangle(0, 0, s, s), this.selHandleConfig.fillColor, this.selHandleConfig.strokeColor);
            }
        }
        ...
        private createLabelHandleShape() : Shape {
            if (this.labelHandleImage != null) {
                ...
                return new RectangleShape(new Rectangle(0, 0, s, s), Constants.labelHandleFillcolor, this.selHandleConfig.strokeColor);
            }
        }
        ...
        initBend(bend: Shape) {
            if (this.preferHtml) {
                ...
                bend.init(ElementInitializer(this.graph.getView().getOverlayPane()));
            }
        }
        ...
        private start(x: number, y: number, index: number) {
            ...
            if (this.isSource || this.isTarget) {
                var cell = this.state.cell;
                var terminal = Cells.getTerminal(cell, this.isSource);
        ...
        private getPointForEvent(me: MouseEventContext) {
            ...
            if (tt > 0 && this.isSnapToTerminalsEvent(me)) {
                ...
                // Temporary function
	            var snapToTerminal = (terminal: CellState): void => {
                    if (terminal != null) {
                ...
                if (me.getX() != this.startX || me.getY() != this.startY) {
                    ...
                    if (this.error != null) {
                        ...
                        if (terminal != null) {
                            ...
                            var pstate = this.graph.getView().getState(Cells.getParent(edge));
        ...
        convertPoint(point: Point, gridEnabled: boolean) : Point {
            ...
            var pstate = this.graph.getView().getState(
                Cells.getParent(this.state.cell));
        ...
        private connect(edge: Cell, terminal: Cell, isSource: boolean, isClone: boolean, me: MouseEventContext) {
            ...
            try {
                ...
                if (isClone) {
                    ...
                    model.add(parent, clone, Cells.getChildCount(parent));

                    var other = Cells.getTerminal(edge, !isSource);
        ...
        private changeTerminalPoint(edge: Cell, point: Point, isSource: boolean, clone: boolean) {
            ...
            try {
                if (clone) {
                    var parent = Cells.getParent(edge);
                    var terminal = Cells.getTerminal(edge, !isSource);
                    edge = this.graph.cloneCells([edge])[0];
                    model.add(parent, edge, Cells.getChildCount(parent));
        ...
        private getHandleFillColor(index: number) : string {
            ...
            var cell = this.state.cell;
            var terminal = Cells.getTerminal(cell, isSource);
