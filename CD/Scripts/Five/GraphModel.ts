...
module Five {
    ...
    export class GraphModel implements IUndoableEditListener {
        ...
        /**
         * Counter for the depth of nested transactions. Each call to <beginUpdate> will increment this number and each call to <endUpdate> will decrement
         * it. When the counter reaches 0, the transaction is closed and the respective events are fired. Initial value is 0. */
        ...
        add(parent: Cell, child: Cell, index?: number): Cell {
            ...
            if (child != parent && parent != null && child != null) {
                ...
                var parentChanged = parent != Cells.getParent(child);
        ...
        parentForCellChanged(cell: Cell, parent: Cell, index: number): Cell {
            ...
            var previous = Cells.getParent(cell);
        ...
        terminalForCellChanged(edge: Cell, terminal: Cell, isSource: boolean): Cell {
            ...
            var previous = Cells.getTerminal(edge, isSource);
        ...
        private updateEdgeParent(edge: Cell, root: Cell) {
            ...
            var target = Cells.getTerminal(edge, false);
            var cell: Cell;

            // Uses the first non-relative descendants of the source terminal
            while (source != null && !Cells.isEdge(source) &&
                source.geometry != null && source.geometry.relative) {
                source = Cells.getParent(source);
            }

            // Uses the first non-relative descendants of the target terminal
            while (target != null && !Cells.isEdge(target) &&
                target.geometry != null && target.geometry.relative) {
                target = Cells.getParent(target);
            }

            if (Cells.isAncestor(root, source) && Cells.isAncestor(root, target)) {
                if (source == target) {
                    cell = Cells.getParent(source);
                } else {
                    cell = this.getNearestCommonAncestor(source, target);
                }

                if (cell != null && (Cells.getParent(cell) !== this.root ||
                    Cells.isAncestor(cell, edge)) && Cells.getParent(edge) !== cell) {
                    var geo = Cells.getGeometry(edge);

                    if (geo != null) {
                        var origin1 = this.getOrigin(Cells.getParent(edge));
                    ...
                    this.add(cell, edge, Cells.getChildCount(cell));
                }
            }
        }
        ...
        private updateEdgeParents(cell: Cell, root?: Cell) {
            ...
            for (i = 0; i < edgeCount; i++) {
                edges.push(Cells.getEdgeAt(cell, i));
            }
        ...
        private contains(cell: Cell): boolean {
            return Cells.isAncestor(this.root, cell);
        }
        ...
        private cellAdded(cell: Cell) {
            ...
            if (cell != null) {
                ...
                for (var i = 0; i < childCount; i++) {
                    this.cellAdded(Cells.getChildAt(cell, i));
                }
            }
        }
        ...
        private getOrigin(cell: Cell): Point {
            ...
            if (cell != null) {
                result = this.getOrigin(Cells.getParent(cell));
        ...
        setTerminal(edge: Cell, terminal: Cell, isSource: boolean) : void{
            ...
            var terminalChanged = terminal != Cells.getTerminal(edge, isSource);
        ...
        setCollapsed(cell: Cell, collapsed: boolean): boolean {
            ...
            if (collapsed !== Cells.isCollapsed(cell)) {
        ...
        collapsedStateForCellChanged(cell: Cell, collapsed: boolean): boolean {
            ...
            var previous = Cells.isCollapsed(cell);
        ...
        getEdgesBetween(source: Cell, target: Cell, directed: boolean): Cell[] {
            ...
            var tmp1 = Cells.getEdgeCount(source);
            var tmp2 = Cells.getEdgeCount(target);
            ...
            for (var i = 0; i < edgeCount; i++) {
                var edge = Cells.getEdgeAt(terminal, i);
                var src = Cells.getTerminal(edge, true);
                var trg = Cells.getTerminal(edge, false);
        ...
        getDirectedEdgeCount(cell: Cell, outgoing: boolean, ignoredEdge?: Cell): number {
            ...
            for (var i = 0; i < edgeCount; i++) {
                var edge = Cells.getEdgeAt(cell, i);

                if (edge != ignoredEdge && Cells.getTerminal(edge, outgoing) == cell) {
                    count++;
                }
            }
            ...
            if (includeChildren) {
                ...
                for (var i = 0; i < childCount; i++) {
                    var cloneChild = this.cloneCellImpl(
                        Cells.getChildAt(cell, i), mapping, true);
        ...
        private restoreClone(clone: Cell, cell: Cell, mapping: Dictionary<Cell, Cell>) {
            var source = Cells.getTerminal(cell, true);
            var tmp: Cell;
            if (source != null) {
            ...
            var target = Cells.getTerminal(cell, false);
            ...
            var childCount = Cells.getChildCount(clone);

            for (var i = 0; i < childCount; i++) {
                this.restoreClone(Cells.getChildAt(clone, i),
                    Cells.getChildAt(cell, i), mapping);
            }
        }

        /** Sets the style of the given Cell using StyleChange and adds the change to the current transaction. */
        setStyle(cell: Cell, style: AppliedStyle): void {
            if (style != Cells.getStyle(cell)) {
                this.execute(new StyleChange(this, cell, style));
            }
        }

        styleForCellChanged(cell: Cell, style: AppliedStyle): AppliedStyle {
            ...
            var previous = Cells.getStyle(cell);
            cell.setStyle(style);
        ...
        isLayer(cell: Cell): boolean {
            return this.isRoot(Cells.getParent(cell));
        }
        ...
        getChildCells(parent: Cell, vertices: boolean, edges: boolean): Cell[] {
            ...
            for (var i = 0; i < childCount; i++) {
                ...
                if ((!edges && !vertices) || (edges && Cells.isEdge(child)) ||
                    (vertices && Cells.isVertex(child))) {
                    result.push(child);
                }
            }
        ...
        visibleStateForCellChanged(cell: Cell, visible: boolean): boolean {
            ...
            var previous = Cells.isVisible(cell);
        ...
        * cell and all of its children from the model. Returns the removed cell. */
        remove(cell: Cell): Cell {
            if (cell == this.root) {
                this.setRoot(null);
            } else if (Cells.getParent(cell) != null) {
        ...
        * cells - Array of cells whose parents should be returned. */
        ...
        setCellStyles(cells: Cell[], setter: StyleSetter) {
            ...
            if (cells != null && cells.length > 0) {
                ...
                try {
                    for (var i = 0; i < cells.length; i++) {
                        if (cells[i] != null) {
                            var appliedStyle = Cells.getStyle(cells[i]);
                            setter(appliedStyle);
