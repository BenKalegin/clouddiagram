...
module Five {
    ...
    export class Utils {
         ...
         * ObjectIdentity.FIELD_NAME is always ignored by this function.
         * @param obj Object to be cloned.
         * @param transients Optional array of strings representing the fieldname to be ignored
         * @param shallow shallow - Optional boolean argument to specify if a shallow clone should be created, that is,
         * one where all object references are not cloned or, in other words, one where only atomic(strings, numbers) values are cloned.Default is false.
         */
        static clone<T extends object>(obj: T, transients?: string[], shallow: boolean = null): T {
                ...
                for (var key in obj) {
                    if (key != ObjectIdentity.fieldName && (transients == null || Utils.indexOf(transients, key) < 0)) {
                        if (!shallow && typeof (obj[key]) == "object") {
                            let subobj = <object><any>obj[key];
                            (<any>clone)[key] = this.clone(subobj);
                        } else {
                            clone[key] = obj[key];
                        }
                    }
        ...
        static bind<TFunc extends Function, TObject>(scope: TObject, funct: TFunc): TFunc {
            return <any>(function() { return funct.apply(scope, arguments) });
        }
        ...
        private static prefix: string =
            (Client.isOp && Client.isOt)
                ? "O"
                : (Client.isSf || Client.isGc)
                ? "Webkit"
                : (Client.isMt)
                ? "Moz"
                : (Client.isIe9 && !Client.isIe10)
                ? "ms"
                : null;
        ...
        static getAlignmentAsPoint(align: HorizontalAlign, valign: VerticalAlign): Point {
            var dx = 0;
            var dy = 0;
        ...
        static getDocumentScrollOrigin(doc: Document): Point {
            ...
            if (Client.isQuirks) {
                ...
                var x = (wnd != null && window.pageXOffset !== undefined)
                    ? window.pageXOffset
                    : (document.documentElement || <Element>document.body.parentNode || document.body).scrollLeft;
                var y = (wnd != null && window.pageYOffset !== undefined)
                    ? window.pageYOffset
                    : (document.documentElement || <Element>document.body.parentNode || document.body).scrollTop;
...
/**
        ...
        static intersectsHotspot(state: CellState, x: number, y: number, hotspot: number, min: number, max: number):
            boolean {
        ...
        static findNearestSegment(state: CellState, x: number, y: number) {
            ...
            if (state.absolutePoints.length > 0) {
                ...
                for (var i = 1; i < state.absolutePoints.length; i++) {
                    ...
                    var dist = Utils.ptSegDistSq(last.x,
                        last.y,
                        current.x,
                        current.y,
                        x,
        ...
        * getDropTarget - Optional function to return the drop target for a given location(x, y).Default is mxGraph.getCellAt. */
        static makeDraggable(element: HTMLElement,
            ...
            dx: number = 0,
            dy: number = Constants.tooltipVerticalOffset,
            autoscroll?: boolean,
            scalePreview?: boolean,
            highlightDropTargets?: boolean,
            getDropTarget?: (graph: Graph, x: number, y: number, evt: MouseEvent) => Cell) {
        ...
        static findNodeByAttribute(node: Node, attr: string, value: string): Node {
            ...
            if (Client.isIe9) {
                ...
                if (node == null) {
                    ...
                    var result1 = (<any>node.ownerDocument).evaluate('//*[@' + attr + '=\'' + value + '\']',
                        node.ownerDocument,
                        null,
                        0,
                        null);
	 ...
	 * allChildren - If all children should be imported. */
                ...
                case NodeType.Element:
                {
                    ...
                    if (element.attributes && element.attributes.length > 0) {
                        ...
                        for (i = 0; i < element.attributes.length; i++) {
                            newNode.setAttribute(element.attributes[i].nodeName,
                                element.getAttribute(element.attributes[i].nodeName));
                        }
        ...
        static error(message: string, width: number, close: boolean, icon?: string) {
            ...
            var warn = new Window(Resources.get(Utils.errorResource),
                div,
                (w - width) / 2,
                h / 4,
                width,
    ...
    class TemporaryCellStates {
        constructor(view: GraphView, scale: number = 1, cells?: Cell[], isCellVisibleFn?: ICellVisible) {
            ...
            view.validateCellState = (cell: Cell, recurse: boolean = true): CellState => {
