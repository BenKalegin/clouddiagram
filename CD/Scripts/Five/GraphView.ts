module Five {
    ...
    export class GraphView implements IUndoableEditListener{
        graph: Graph;

        onChange: EventListeners<ModelChangeEvent> = null;
        onAfterExecute: EventListeners<AfterExecuteEvent> = null;
        onStartEdit: EventListeners<BasicEvent> = null;
        onEndEdit: EventListeners<BasicEvent> = null;
        onNotify: EventListeners<NotifyEvent> = null;
        ...
        private isEventsEnabled_ = true;

        public get isEventsEnabled(): boolean {return this.isEventsEnabled_}
        public set isEventsEnabled(value: boolean) {this.isEventsEnabled_ = value}
        ...
        clear(cell?: Cell, force?: boolean, recurse?: boolean) {
            ...
            if (recurse && (force || cell != this.currentRoot)) {
                ...
                for (var i = 0; i < childCount; i++) {
                    this.clear(Cells.getChildAt(cell, i), force);
                }
            } else {
        ...
        invalidate(cell?: Cell, recurse?: boolean, includeEdges?: boolean) {
            ...
            if (cell && !cell.invalidating) {
                ...
                if (includeEdges) {
                    var edgeCount = Cells.getEdgeCount(cell);

                    for (var j = 0; j < edgeCount; j++) {
                        this.invalidate(Cells.getEdgeAt(cell, j), recurse, includeEdges);
                    }
                }
        ...
        getBoundingBox(state: CellState, recurse: boolean = true) : Rectangle{
            ...
            if (state != null) {
                ...
                if (recurse) {
                    var childCount = Cells.getChildCount(state.cell);

                    for (var i = 0; i < childCount; i++) {
                        var bounds = this.getBoundingBox(this.getState(Cells.getChildAt(state.cell, i)));
        ...
        validateBackgroundImage() {
            ...
            if (bg != null) {
                if (this.backgroundImage == null || this.backgroundImage.image !== bg.src) {
                    ...
                    this.backgroundImage.init(ElementInitializer(this.backgroundPane));
        ...
        validateBackgroundPage() {
            if (this.graph.pageVisible) {
                ...
                if (this.backgroundPageShape == null) {
                    ...
                    this.backgroundPageShape.init(ElementInitializer(this.backgroundPane));
                    this.backgroundPageShape.redraw();

                    // Adds listener for double click handling on background
                    if (this.graph.nativeDblClickEnabled) {
                        Events.addListener(this.backgroundPageShape.node, "dblclick", Utils.bind(this, (evt) => { this.graph.dblClick(<MouseEvent>evt); }));
                    }
        ...
        validateCell(cell: Cell, visible: boolean = true) {
            if (cell != null) {
                ...
                if (state != null && !visible) {
                    ...
                    for (var i = 0; i < childCount; i++) {
                        this.validateCell(Cells.getChildAt(cell, i), visible &&
                ...
                if (state != null) {
                    if (state.invalid) {
                        ...
                        if (cell != this.currentRoot) {
                            this.validateCellState(Cells.getParent(cell), false);
                        }
                    ...
                    if (recurse) {
                        ...
                        for (var i = 0; i < childCount; i++) {
                            this.validateCellState(Cells.getChildAt(cell, i));
                        }
                    }
                }
            }
        ...
        updateCellState(state: CellState) {
            ...
            if (state.cell != this.currentRoot) {
                var pState = this.getState(Cells.getParent(state.cell));
        ...
        updateVertexState(state: CellState, geo: Geometry) {
            ...
            if (geo.relative && pState != null && !Cells.isEdge(pState.cell)) {
        ...
        updateEdgeState(state: CellState, geo: Geometry) {
            ...
            if ((Cells.getTerminal(state.cell, true) != null && source == null) ||
            (source == null && geo.getTerminalPoint(true) == null) ||
				(Cells.getTerminal(state.cell, false) != null && target == null) ||
        ...
        updateVertexLabelOffset(state: CellState) {
            var h = state.style.labelPosition || HorizontalAlign.Center;
            var lw: number;
            ...
            } else if (h == HorizontalAlign.Center) {
                lw = state.style.labelWidth;
                if (lw != null) {
            ...
            var v = state.style.verticalLabelPosition || VerticalAlign.Middle;
        ...
        stateValidated(state: CellState) {
            var fg = (Cells.isEdge(state.cell) && this.graph.keepEdgesInForeground) ||
                (Cells.isVertex(state.cell) && this.graph.keepEdgesInBackground);
        ...
        updateFloatingTerminalPoint(edge: CellState, start: CellState, end: CellState, source: boolean) {
            ...
            var alpha = Utils.toRadians(start.style.rotation);
        ...
        getTerminalPort(state: CellState, terminal: CellState, source: boolean): CellState {
            ...
            var id = null; //state.style.sourcePort, key, null);
        ...
        getVisibleTerminal(edge: Cell, source: boolean): Cell {
            var model = this.graph.getModel();
            var result = Cells.getTerminal(edge, source);
            var best = result;
            ...
            while (result != null && result != this.currentRoot) {
                ...
                result = Cells.getParent(result);
            }

            // Checks if the result is not a layer
            if (Cells.getParent(best) == model.getRoot()) {
                best = null;
            }
        ...
        createState(cell: Cell): CellState {
            ...
            if (state.view.graph.container != null && state.cell != state.view.currentRoot &&
                (Cells.isVertex(state.cell) || Cells.isEdge(state.cell))) {
        ...
        isScrollEvent(evt: MouseEvent): boolean {
            ...
            var pt = new Point(evt.clientX - totalOffset.x, evt.clientY - totalOffset.y);
        ...
        private installListeners(): void {
            ...
            if (container != null) {
                ...
                if (Client.isTouch) {
                    Events.addListener(container.eventTarget(), "gesturestart", Utils.bind(this, evt => {
                        graph.fireGestureEvent(evt as MouseEvent);
                        Events.consume(evt);
                    }));

                    Events.addListener(container.eventTarget(), "gesturechange", Utils.bind(this, evt => {
                        graph.fireGestureEvent(evt as MouseEvent);
                        Events.consume(evt);
                    }));

                    Events.addListener(container.eventTarget(), "gestureend", Utils.bind(this, evt => {
                        graph.fireGestureEvent(evt as MouseEvent);
                        Events.consume(evt);
                    }));
                }
                ...
                Events.addGestureListeners(container.eventTarget(), (evt: MouseEvent) => {
                ...
                Events.addListener(container.eventTarget(), "dblclick", Utils.bind(this, (evt: MouseEvent) => {
                    if (this.isContainerEvent(evt)) {
                        graph.dblClick(evt);
                    }
                }));
                ...
                this.moveHandler = Utils.bind(this, (evt: MouseEvent) => {
                    ...
                    if (this.captureDocumentGesture && graph.isMouseDown && graph.container != null &&
                        !this.isContainerEvent(evt) && graph.container.isVisible() && !Events.isConsumed(evt)) {
                ...
                this.endHandler = Utils.bind(this, (evt: MouseEvent) => {
                    if (this.captureDocumentGesture && graph.isMouseDown && graph.container != null &&
                        !this.isContainerEvent(evt) && graph.container.isVisible()) {
                        graph.fireMouseEvent(Events.mouseUp, new MouseEventContext(evt));
                    }
                });
        ...
        destroy() {
            ...
            if (root != null && root.parentNode != null) {
                ...
                Events.release(this.graph.container.eventTarget());
