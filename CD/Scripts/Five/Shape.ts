module Five {
    ...
    export interface IShapeInitializer {
        isSvg() : boolean;
        addShape(shape: Element);
    }

    export function ElementInitializer(container: Element): IShapeInitializer {
        return {
            isSvg: () => ((<SVGElement><any>container).ownerSVGElement != null),
    ...
    export class Shape {
        ...
        endSize: number;
        private startArrow: ArrowStyle;
        private endArrow: ArrowStyle; 
        ...
        private create(initializer: IShapeInitializer) : Element {
            /// <summary>Creates and returns the DOM node(s) for the shape in the given container. This implementation invokes createSvg or createHtml depending on the dialect and style settings.</summary>
            /// <param name="container" type="">DOM node that will contain the shape</param>
        ...
        protected createDiv(): HTMLDivElement {
            /// <summary>Creates and returns the HTML DOM node(s) to represent this shape.</summary>
        ...
        private updateHtmlBounds(node) {
            var sw = (Client.isIe9) ? 0 : Math.ceil(this.strokewidth * this.scale);
        ...
        private destroyCanvas(canvas) {
            ...
            if (canvas instanceof SvgCanvas2D) {
                ...
                for (var i = 0; i < gradients.length; i++) {
                    ...
                    (<any>gradient).mxRefCount = ((<any>gradient).mxRefCount || 0) + 1;
                }
         ...
         */
        paintGlassEffect(c: AbstractCanvas2D, x, y, w, h, arc) {
            var sw = Math.ceil(this.strokewidth / 2);
            var size = 0.4;

            c.setGradient("#ffffff", "#ffffff", x, y, w, h * 0.6, Direction.South, 0.9, 0.1);
            c.begin();
            arc += 2 * sw;
        ...
        apply(state: CellState) {
            ...
            if (this.style != null) {
                ...
                // Arrow stroke width is used to compute the arrow heads size in Connector
        ...
        /** Sets a transparent background CSS style to catch all events. Paints the line shape. */
