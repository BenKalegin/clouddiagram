module Five {
    ...
    export interface IEdgeFactory {
        (source: Cell, target: Cell, style?: AppliedStyle) : Cell;    
    }
    ...
    export class ConnectionHandler implements IMouseListener {
        ...
        createMarker(): CellMarker {
            ...
            marker.getCell = (me: MouseEventContext) => {
                var cell =  CellMarker.prototype.getCell.apply(marker, [me]);
            ...
            marker.isValidState = (state: CellState) => {
                return this.isConnecting() ? this.error == null : CellMarker.prototype.isValidState.apply(marker, state);
            };

            // Overrides to use marker color only in highlight mode or for
            // target selection
            marker.getMarkerColor = (evt: MouseEvent, state: CellState, isValid: boolean) : string => {
                return (this.connectImage == null || this.isConnecting()) ?
                    CellMarker.prototype.getMarkerColor.apply(marker, [evt, state, isValid]) : null;
            };

            // Overrides to use hotspot only for source selection otherwise
            // intersects always returns true when over a cell
            marker.intersects = (state: CellState, me: MouseEventContext) : boolean => {
                if (this.connectImage != null || this.isConnecting()) {
                    return true;
                }

                return CellMarker.prototype.intersects.apply(marker, [state, me]);
            };
        ...
        private createIcons(state: CellState): ImageShape[] {
            ...
            if (image != null && state != null) {
                ...
                if (this.isMoveIconToFrontForState(state)) {
                    ...
                    icon.init(ElementInitializer(this.graph.getView().getOverlayPane()));
                ...
                try {
                    if (source != null && target == null && this.isCreateTarget()) {
                        ...
                        if (target != null) {
                            ...
                            if (dropTarget == null || !Cells.isEdge(dropTarget)) {
                    ...
                    if (source != null && target != null &&
                        Cells.getParent(source) == Cells.getParent(target) &&
                        Cells.getParent(Cells.getParent(source)) != model.getRoot()) {
                        parent = Cells.getParent(source);
        ...
        private insertEdge(parent: Cell, id: number, value: Node, source: Cell, target: Cell, style?: AppliedStyle): Cell {
         ...
         */
        private createEdge(value: Node, source: Cell, target: Cell, style: AppliedStyle): Cell {
            var edge = null;
