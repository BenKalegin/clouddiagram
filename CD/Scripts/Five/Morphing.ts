     ...
     * delay - Optional delay between the animation steps. Passed to <mxAnimation>. */
        ...
        private animateCell(cell: Cell, move: CellStatePreview, recurse: boolean) {
            ...
            if (state != null) {
                ...
                if (Cells.isVertex(cell) && (delta.x != 0 || delta.y != 0)) {
            ...
            if (recurse && !this.stopRecursion(state, delta)) {
                ...
                for (var i = 0; i < childCount; i++) {
                    this.animateCell(Cells.getChildAt(cell, i), move, recurse);
                }
            }
        }
    ...
    /** Returns true if this contains no entries. */
        ...
        private translateState(state: CellState, dx: number, dy: number) {
            if (state != null) {
                ...
                for (var i = 0; i < childCount; i++) {
                    this.translateState(state.view.getState(Cells.getChildAt(state.cell, i)), dx, dy);
                }
            }
        }
        ...
        private revalidateState(state: CellState, dx: number, dy: number, visitor: (state: CellState) => void) {
            if (state != null) {
                ...
                var pState = state.view.getState(Cells.getParent(state.cell));

                // Moves selection vertices which are relative
                if ((dx != 0 || dy != 0) && geo != null && geo.relative &&
                    Cells.isVertex(state.cell) && (pState == null ||
                    Cells.isVertex(pState.cell) || this.deltas.get(state.cell) != null)) {
                    state.x += dx;
                    state.y += dy;
                }
                ...
                for (var i = 0; i < childCount; i++) {
                    this.revalidateState(this.graph.view.getState(Cells.getChildAt(state.cell, i)), dx, dy, visitor);
                }
            }
        }
        ...
        private addEdges(state: CellState) {
            var model = this.graph.getModel();
            var edgeCount = Cells.getEdgeCount(state.cell);

            for (var i = 0; i < edgeCount; i++) {
                var s = state.view.getState(Cells.getEdgeAt(state.cell, i));
