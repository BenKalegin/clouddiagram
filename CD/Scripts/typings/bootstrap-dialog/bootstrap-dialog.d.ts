...
interface IBootstrapDialog {
    ...
    SIZE_NORMAL: string;
	SIZE_SMALL: string;
	SIZE_WIDE: string;    // size-wide is equal to modal-lg
	SIZE_LARGE: string;
...
interface IBootstrapDialogInstance {
    ...
    $modalHeader: JQuery;
    $modalFooter: JQuery;
    getButton(id: string): IBootstrapDialogLiveButton; 
    getModalFooter(): IBootstrapDialogFooter; 
...
interface IBootstrapDialogFooter {
    hide();
}
...
interface IBootstrapDialogLiveButton {
    toggleEnable(enable: boolean);
    enable();
    disable();
    spin();
    stopSpin();
    toggleSpin(spin: boolean);
}